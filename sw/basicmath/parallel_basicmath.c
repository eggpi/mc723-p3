...
typedef void *(*pthread_start_function_t)(void *);

static int thread_id;
static int pthread_create_lock;

static pthread_start_function_t next_start_function;
static void* next_start_arg;

int
pthread_create(pthread_t *thread, pthread_attr_t attr,
        pthread_start_function_t start_function, void *arg) {

 pthread_create_lock = 1;
 *thread = 0xDEADBEEF;
 next_start_function = start_function;
 next_start_arg = arg;

 uint32_t *p =  (uint32_t *) 0xFFFFFFFF;
 *p = (uint32_t) 1 << ++thread_id;

 while (pthread_create_lock);
 return 0;
}
...
pthread_join(pthread_t thread, void **retval) {
    while (finished_threads < no_workers);
}
...
void createWorkers(){
	...
	for ( index= 0 ; index<no_workers; index++){
				pthread_create(&workers[index],0,doWork,(void *)index);
		 	}
